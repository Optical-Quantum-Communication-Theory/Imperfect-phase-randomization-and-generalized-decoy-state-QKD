...
function [Dfval,realEpsilon] = primalDfep(rho, keyMap, krausOperators, options)
	...
	if nargin < 3 || isempty(krausOperators)
                ...
                if eigMin2 <=0
                    ...
                    % epsilon value
                     ...
                     if realEpsilon < 0 || realEpsilon > 1/(exp(1)*(dim-1))
                        ME = MException('primalDfep:badRho','Please check your rho from the first step calculation. It is not suitable for the second step calculation');
                     	throw(ME);
                     else
                         rhoPrime = (1-realEpsilon) * rho + realEpsilon * eye(dim)/dim;
        ...
        if eigMin4 <= 0
            ...
            if eigMin5<=0
            	ME = MException('primalDfep:badRhobadKeymap','Please check your rho and key map');
                throw(ME);
            end
        ...
        if eigMin <= 0
            ...
            if eigMin3<=0
            	ME = MException('primalDfep:badRhobadEpsilon','Please check your rho and epsilon');
                throw(ME);
            end
            gRho = gRhoPrime;
        end
